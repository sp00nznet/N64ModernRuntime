...
namespace recomp {
    // 4GB read/write — required because PHYS_ADDR can produce values up to
    // 0x7FFFFFFF for addresses near 0x80000000 + small offset, and the guard
    // region fix adds 4KB at the start. OOB accesses land in valid memory.
