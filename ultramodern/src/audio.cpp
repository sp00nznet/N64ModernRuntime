...
void ultramodern::queue_audio_buffer(RDRAM_ARG PTR(int16_t) audio_data_, uint32_t byte_count) {
    ...
    audio_buf_count++;
    if (audio_buf_count <= 5 || (audio_buf_count % 500) == 0) {
        fprintf(stderr, "[AUDIO-BUF] #%d: n64addr=0x%08X phys=0x%06X bytes=%u samples=%u\n",
                audio_buf_count, (uint32_t)audio_data_, phys, byte_count, sample_count);
        fflush(stderr);
    }
